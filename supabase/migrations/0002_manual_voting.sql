-- ============================================================================
--  ЕДИНСТВЕННАЯ миграция. Больше запускать нечего.
--
--  Прогоняется целиком на любой базе — и на пустой, и на той, где осталась
--  старая схема под ApiPay: сначала создаётся новое, потом удаляется лишнее.
--  Предыдущая миграция удалена, чтобы не сбивать с толку.
--
--  ВНИМАНИЕ: teams пересоздаётся с нуля, поэтому повторный прогон вернёт
--  города и ссылки Kaspi к значениям ниже. Если ссылки меняли в админке —
--  сначала перенесите их сюда.
--
...
insert into teams (slug, name, place_label, color_index, kaspi_url, display_order) values
  ('astana',   'Астана қаласы',   'Финалист', 1, 'https://pay.kaspi.kz/pay/gyhuj7li', 1),
  ('pavlodar', 'Павлодар облысы', 'Финалист', 3, 'https://pay.kaspi.kz/pay/ap4zlsap', 2);
